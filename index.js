/* ═══════════════════════════════════════════════════
   index.js  —  UTT LOKO  |  Logique Auth + Firestore
   ═══════════════════════════════════════════════════ */

// ── Helpers UI (disponibles avant le module ES) ──────────────────────────
...
function bgClose(e) {
  if (e.target === document.getElementById('overlay')) closeAuth();
}
function switchP(p) {
...
// Expose pour les attributs HTML onclick=""
window.openAuth  = openAuth;
window.closeAuth = closeAuth;
window.bgClose   = bgClose;
window.switchP   = switchP;
...
(async () => {
  ...
  // Expose pour les autres scripts si besoin
  window._db = db;
  window._FS = {
      ...
      if (navActions) {
        navActions.innerHTML = `
          ...
          <a href="dashboard.html"
             style="padding:9px 18px;border-radius:10px;border:1px solid rgba(168,230,61,.3);
                    background:rgba(168,230,61,.08);color:var(--lime);font-size:.85rem;
          ...
          ${u.role === 'admin'
            ? `<a href="admin.html"
                  style="padding:9px 18px;border-radius:10px;border:none;
                         background:var(--lime);color:var(--bg);font-size:.85rem;
                         font-weight:700;text-decoration:none">
                 🛡️ Admin
               </a>`
            : ''}
          <button
            onclick="sessionStorage.removeItem('uttloko_user');location.reload()"
            style="padding:9px 16px;border-radius:10px;border:1px solid rgba(239,68,68,.2);
                   background:transparent;color:#fca5a5;font-size:.82rem;cursor:pointer">
            Déco
          </button>`;
      }
    } catch (e) { /* session corrompue, on ignore */ }
  }
  ...
  /** Redimensionne une image → base64 JPEG 200×200 max, qualité 80 % */
  ...
  window.doLogin = async () => {
    ...
    if (!email || !pass) {
      showErr('l-err', 'Veuillez remplir tous les champs.');
      return;
    }
    setLoad('l-btn', true);
    ...
    try {
      ...
      if (snap.empty) {
        showErr('l-err', 'Aucun compte avec cet email.');
      ...
      if (userData.password !== hashed) {
        showErr('l-err', 'Mot de passe incorrect.');
        setLoad('l-btn', false, 'Accéder à mon espace');
        return;
      }
  ...
  window.doRegister = async () => {
    ...
    if (!nom || !prenom || !contact || !email || !dept || !pass) {
      showErr('r-err', 'Veuillez remplir tous les champs.');
      return;
    }
    if (pass.length < 8) {
      showErr('r-err', 'Mot de passe trop court (8 car. minimum).');
      return;
    }
    setLoad('r-btn', true);
    ...
    try {
      ...
      if (!exist.empty) {
        showErr('r-err', 'Un compte avec cet email existe déjà.');
      ...
      await setDoc(doc(db, 'users', uid), {
        ...
        photoURL:  photoBase64,   // base64 stocké dans Firestore
        password:  hashed,
        role:      'teacher',
